...
class AblationCAM(BaseCAM):
    def __init__(self,
                 ...
                 batch_size: int = 8,           ##########modified from 32 to 1###########
    ...
    def assemble_ablation_scores(self,
        ...
        new_scores = new_scores[sorted_indices]
        ...
        for i in range(number_of_channels):
            ...
            result.append(weight.reshape((1,1)))
        return result
    ...
    def get_cam_weights(self,
        ...
        #         # [target(output).item() for target, output in zip(targets, outputs)])
        #     original_scores = np.float32(targets[0](outputs))
        
        ### modified to use attention loss ###
        outputs = self.model(input_tensor)
        handle.remove()
        original_scores = targets[0](outputs) # tensor of batchsize
        ...
        for batch_index, (target, tensor) in enumerate(
            ...
            for i in tqdm.tqdm(
                ...
                res = self.model(batch_tensor)
                # score = [target(o).item() for o in res]
                score = target(res)
                new_scores.append(score)
                ablation_layer.indices = ablation_layer.indices[batch_tensor.size(
                    0):]
            new_scores = torch.cat(new_scores) #torch.Size([256])
        ...
        return weights ### tensor Size(1,256)
